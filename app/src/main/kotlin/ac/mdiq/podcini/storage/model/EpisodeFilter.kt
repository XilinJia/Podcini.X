...
class EpisodeFilter(vararg properties_: String) : Serializable {
    ...
    fun queryString(): String {
        ...
        if (propertySet.contains(States.unspecified.name)) stateQuerys.add(" playState == ${PlayState.UNSPECIFIED.code} ")
        if (propertySet.contains(States.building.name)) stateQuerys.add(" playState == ${PlayState.BUILDING.code} ")
        if (propertySet.contains(States.new.name)) stateQuerys.add(" playState == ${PlayState.NEW.code} ")
        if (propertySet.contains(States.unplayed.name)) stateQuerys.add(" playState == ${PlayState.UNPLAYED.code} ")
        if (propertySet.contains(States.later.name)) stateQuerys.add(" playState == ${PlayState.LATER.code} ")
