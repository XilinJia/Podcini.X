...
* In Library
	* fixed Library possibly being locked in view garden mode
...
* in Facets, enhanced "Clear new" operation
* when erase episodes, ensure to remove them from queue
* fixed DB write error that could cause failure in enqueue/download new episodes
* avoided exception in Queues screen when getting Mediabrowser
* amended text in Queues setting: Auto download items in queue
* added some feed/episode specific logging
...
* moved saving episodes to IO dispatcher at receiver device 
* fixed bug in checking whether feed is in normal volume, that affects feeds in a sub-volume
...
* send catalog is temporarily disabled (need proper handling of duplicate feeds)
...
* in Queues screen
	* curQueue flow is amended for bin list live updates
	* Queue and Bin have separate episodes flows 
* corrected bottom padding of PlayerDetailed
* ensure some null checking
* moved main compose screen out of MainActivity
* coil is upgraded to version 3
...
* moved some db monitorings from MainActivity to the App
* replaces reference to mipmap properties
* VideoMode only has: Default, Video, Audio only. 
	* in Feed Settings, Default means using Global
	* window or full screen is handled by rotating the device
* reduced some screens' outer horizontal padding
* old shared_prefs directory is removed
